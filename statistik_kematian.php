...
if(isset($_POST['qcari'])) {
	$qcari=$_POST['qcari'];
	$mySql="SELECT * FROM surat_kematian WHERE tgl_surat like '%$qcari%' ORDER BY tgl_surat DESC";
	$mySqlChrt="SELECT * FROM surat_kematian WHERE tgl_surat like '%$qcari%' GROUP BY DATE_FORMAT(tgl_surat, '%Y - %m')";
	$count= "SELECT COUNT(id) as jumlah FROM surat_kematian WHERE tgl_surat like '%$qcari%' GROUP BY DATE_FORMAT(tgl_surat, '%Y - %m')";
}
?>
<div>
...
<div class="card">
	...
	<div class="form-group">
			...
			<div class="right"  style="margin-right:10px;">
					<form class="" method="POST" action="statistik_kematian.php">
			...
			<?php $mycount = mySqli_query ($koneksi, $count) or die ("Query salah : ".mysqli_error ($koneksi)); ?>
					...
					var myChart = new Chart(ctx, {
							...
							data: {
									...
									datasets: [{
													label: 'Jumlah Pengajuan Surat Kematian : <?php echo mysqli_num_rows($myQry1); ?>',
