...
<?php
	$id = $_GET ['id'];
	$sql = mysqli_query($koneksi, "SELECT * FROM surat_tidakmampu WHERE id='$id' AND jenis=1");
	...
	if(isset($_POST['add'])){
		...
		//tgl_surat='$tgl_surat'
		$update = mysqli_query($koneksi, "UPDATE surat_tidakmampu SET status_surat='$status_surat',ket_surat='$ket_surat',tgl_surat='$tgl_surat' WHERE id='$id'") or die(mysqli_error());
		if ($update) {
		?>
		...
		<meta http-equiv='refresh' content='3; url=surat_miskin_rs.php'/>
		<?php
		}else{
	...
	?>
		<h2>Data Surat miskin_rs &raquo; Proses</h2>
		<hr />
	...
	<div class="form-group">
		...
		<input type="text" name="nama" value="<?php echo $row['nama'] ?>" class="form-control"
			...
			<div class="form-group">
				...
				<input type="text" name="pekerjaan" value="<?php echo $row['pekerjaan'] ?>" class="form-control" placeholder="Alamat" disabled></required>
			</div>
			<div class="form-group">
				<label class="control-label">Untuk</label>
				<input type="text" name="atas_nama" value="<?php echo $row['atas_nama'] ?>" class="form-control" placeholder="Alamat" disabled></required>
			</div>
			<div class="form-group">
			...
			<div class="form-group">
				...
				<a class="btn btn-danger" href="surat_miskin_rs.php">Kembali</a>
		</div>
  </div>
</form>
