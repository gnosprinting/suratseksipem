...
<?php
    ...
    $tempat_lhr	= $row['tempat_lhr'];
					...
					if ($ukuran_ktp < (2*1024*1024)) {
						if (in_array($ekstensi_kk, $ekstensi_diperbolehkan) === true) {
							if ($ukuran_kk < (2*1024*1024)) {
								...
								//insert to database
								$insert = mysqli_query($koneksi, "INSERT INTO surat_belum_memiliki_rumah( no_surat,nama,nik,tgl_lhr,tempat_lhr,jk,status,negara,agama,pekerjaan,alamat,rt_rw,keperluan,s_kk,s_ktp,s_pengantar,s_pernyataan,ktp_saksi1,ktp_saksi2,status_surat)
								VALUES('$no_surat','$nama','$nik','$tanggal','$tempat_lhr','$jk','$status','$negara','$agama','$pekerjaan','$alamat','$rt_rw','$keperluan','$s_kk','$s_ktp','$s_pengantar','$s_pernyataan','$s_saksi1','$s_ktp_saksi2','$status_surat')") or die (mysqli_error ($koneksi));
	    ...
	    ?>
		<h2>Data Surat Belum Memiliki Rumah &raquo; Tambah Data</h2>
		<hr />
  ...
  <div class="row">
    ...
    <div class="col">
			<?php
				$sqli = mysqli_query($koneksi, "SELECT * FROM surat_belum_memiliki_rumah");
				$jml = mysqli_num_rows($sqli);
				$nos = $jml+1;
			?>
			<div class="form-group">
				<label class="control-label">No Surat</label>
				<input type="text" name="no_surat" value="501/<?php echo $nos; ?>" class="form-control" readonly="true"></required>
			</div>
			<div class="form-group">
