...
<?php
					...
					if ($ukuran_ktp < (2*1024*1024)) {
						if (in_array($ekstensi_kk, $ekstensi_diperbolehkan) === true) {
							if ($ukuran_kk < (2*1024*1024)) {
								...
								//insert to database
								$insert = mysqli_query($koneksi, "INSERT INTO surat_duda_janda(no_surat,nik,nama,tempat_lhr,tgl_lhr,jk,status,negara,det_status,agama,pekerjaan,alamat,rt_rw,mantan,s_kk,s_ktp,s_pengantar,s_kematian,s_pbb,s_cerai,status_surat)
								VALUES('$no_surat','$nik','$nama','$tempat_lahir','$tanggal_lahir','$jenis_kelamin','$status','$negara','$det_status','$agama','$pekerjaan','$alamat','$rt_rw','$mantan','$s_kk','$s_ktp','$s_pengantar','$s_kematian','$s_pbb','$s_cerai','$status_surat')") or die (mysqli_error ($koneksi));
  ...
  <div class="row">
    ...
    <div class="col">
		...
		-->
		<?php
			$sqli = mysqli_query($koneksi, "SELECT * FROM surat_duda_janda");
			$jml = mysqli_num_rows($sqli);
			$nos = $jml+1;
		?>
		<div class="form-group">
			<label class="control-label">No Surat</label>
			<input type="text" name="no_surat" value="400/<?php echo $nos; ?>" class="form-control" readonly="true"></required>
		</div>
		<div class="form-group row">
			...
			<input type="text" name="status" value="<?php echo $statusi; ?>" class="form-control" readonly="true"></required>
			</div>
			<div class="col-xs-3">
		...
		<div class="form-group">
			...
			<input type="text" name="negara"  value="Indonesia" class="form-control" placeholder="negara" required>
		</div>
		<div class="form-group">
