...
<?php
    ...
    $tempat_lhr		  = $row['tempat_lhr'];
					...
					if ($ukuran_ktp < (2*1024*1024)) {
						if (in_array($ekstensi_kk, $ekstensi_diperbolehkan) === true) {
							if ($ukuran_kk < (2*1024*1024)) {
								...
								//insert to database
								$insert = mysqli_query($koneksi, "INSERT INTO surat_domisili(no_surat, nama, nik, tgl_lhr, tempat_lhr, jk,agama,pekerjaan,alamat,s_pengantar,s_ktp,s_kk,status_surat)
								VALUES('$no_surat','$nama','$nik','$tanggal', '$tempat_lhr', '$jk','$agama','$pekerjaan', '$alamat','$s_pengantar','$s_ktp','$s_kk','$status_surat')") or die (mysqli_error ($koneksi));
								if($insert){
									...
									<meta http-equiv="refresh" content="5; url=surat_domisili.php"/>';
								}else{
		...
		<hr />
		<form class="form-horizontal" action="surat_domisili_tambah.php?nik=<?php echo $nik; ?>" method="post" enctype="multipart/form-data">
  <div class="row">
			</div>
    <div class="col">
			<?php
				$sqli = mysqli_query($koneksi, "SELECT * FROM surat_domisili");
				$jml = mysqli_num_rows($sqli);
				$nos = $jml+1;
			?>
			<div class="form-group">
				<label class="control-label">No Surat</label>
				<input type="text" name="no_surat" value="471.1/<?php echo $nos; ?>" class="form-control" readonly="true"></required>
			</div>
			<div class="form-group">
				<label class="control-label">Agama</label>
				<input type="text" name="agama" value="<?php echo $row['agama'] ?>" class="form-control" readonly='true'></required>
			</div>
			<div class="form-group">
				<label class="control-label">Pekerjaan</label>
				<input type="text" name="pekerjaan"  value="<?php echo $row['pekerjaan'] ?>" class="form-control" readonly='true'>
			</div>
			<div class="form-group">
			...
			<div class="form-group">
				<label class="control-label">Kartu Keluarga Penannggung Jawab (JPG,JPEG)</label>
