...
<?php
	...
	//$no_kk = $row['no_kk'];
	// $sqli = mysqli_query($koneksi, "SELECT * FROM t_penduduk WHERE nik='$nik'");
	// $rowi = mysqli_fetch_assoc($sqli);
	$st = $row['status_keluarga'];
	//echo $row['nik'];
	if ($st != 'Suami') {
		echo '<div class="alert alert-danger alert-dismissable"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Bukan Kepala Keluarga Dilarang Input.</div>
		<meta http-equiv="refresh" content="3; url=surat_kelahiran.php"/>';
	}
	//Proses Simpan Data
	if(isset($_POST['add'])){
		//print_r($_POST);
					...
					if ($ukuran_ktp_suami < (2*1024*1024)) {
						if (in_array($ekstensi_kk, $ekstensi_diperbolehkan) === true) {
							if ($ukuran_kk < (2*1024*1024)) {
								...
								//insert to database
								$insert = mysqli_query($koneksi, "INSERT INTO surat_kelahiran(no_surat,nik,hari,tanggal,lokasi,jk,nama_anak,nama_ibu,alamat,nama_ayah,s_kk,s_ktp_suami,s_ktp_isteri,kartu_lahir,akta_kakak,akta_ibu,akta_ayah,status_surat)
								VALUES('$no_surat','$nik','$hari','$tanggal','$lokasi','$jk','$nama_anak','$nama_ibu','$alamat','$nama_ayah','$s_kk','$s_ktp_suami','$s_ktp_isteri','$s_kartu_lahir','$s_akta_kakak','$s_akta_ibu','$s_akta_ayah','$status_surat')") or die (mysqli_error ($koneksi));
								if($insert){
	    ...
	    ?>
		<h2>Data Surat Kelahiran &raquo; Tambah Data</h2>
		<hr />
  ...
  <div class="row">
    ...
    <div class="col">
			...
			<?php
				$sqli = mysqli_query($koneksi, "SELECT * FROM surat_kelahiran");
				$jml = mysqli_num_rows($sqli);
				$nos = $jml+1;
			?>
			<div class="form-group">
				<label class="control-label">No Surat</label>
				<input type="text" name="no_surat" value="472.11/<?php echo $nos; ?>" class="form-control" readonly="true"></required>
			</div>
			<div class="form-group">
				<label class=" control-label">Tanggal</label>
				<input type="date" name="tanggal" value="<?php echo date('Y-m-d'); ?>" class="input-group form-control" required>
		  </div>
			<div class="form-group">
