...
<?php
					...
					if ($ukuran_ktp < (2*1024*1024)) {
						if (in_array($ekstensi_kk, $ekstensi_diperbolehkan) === true) {
							if ($ukuran_kk < (2*1024*1024)) {
								...
								//insert to database
								$insert = mysqli_query($koneksi, "INSERT INTO surat_domisili_usaha(no_surat,nik,nama,tempat_lhr,tgl_lhr,jk,status,agama,alamat,rt_rw,nama_usaha,bidang_usaha,tgl_surat,alamat_usaha,nmr_surat,keperluan,s_kk,s_ktp,s_pengantar,s_pernyataan,s_pbb,ktp_saksi1,ktp_saksi2,status_surat)
								VALUES('$no_surat','$nik','$nama','$tempat_lahir','$tanggal_lahir','$alamat_usaha_kelamin','$status','$agama','$alamat','$rt_rw','$nama_usaha','$bidang_usaha','$tgl_surat','$alamat_usaha','$nmr_surat','$keperluan','$s_kk','$s_ktp','$s_pengantar','$s_pernyataan','$s_pbb','$s_saksi1','$s_ktp_saksi2','$status_surat')") or die (mysqli_error ($koneksi));
  ...
  <div class="row">
    ...
    <div class="col">
		...
		<?php
			$sqli = mysqli_query($koneksi, "SELECT * FROM surat_domisili_usaha");
			$jml = mysqli_num_rows($sqli);
			$nos = $jml+1;
		?>
		<div class="form-group">
			<label class="control-label">No Surat</label>
			<input type="text" name="no_surat" value="503/<?php echo $nos; ?>" class="form-control" readonly="true"></required>
		</div>
		<div class="form-group">
		...
		<div class="form-group">
			...
			<input type="text" name="nama_usaha"  value="" class="form-control" placeholder="nama usaha" required>
		</div>
		<div class="form-group">
		...
		<div class="form-group">
			...
			<input type="date" name="tgl_surat"  value="<?php echo date('Y-m-d'); ?>" class="form-control" placeholder="tanggal surat" required>
		</div>
		<div class="form-group">
