...
if(isset($_POST['qcari'])) {
	$qcari=$_POST['qcari'];
	$mySql="SELECT * FROM surat_domisili WHERE tgl_surat like '%$qcari%'";
	$mySqlChrt="SELECT * FROM surat_domisili WHERE tgl_surat like '%$qcari%' GROUP BY tgl_surat";
	$count= "SELECT COUNT(id) as jumlah FROM surat_domisili WHERE tgl_surat like '%$qcari%' GROUP BY tgl_surat";
}
?>
<div>
			...
			<?php $mycount = mySqli_query ($koneksi, $count) or die ("Query salah : ".mysqli_error ($koneksi)); ?>
					...
					var myChart = new Chart(ctx, {
							...
							data: {
									...
									datasets: [{
													label: 'Jumlah Pengajuan Domisili: <?php echo mysqli_num_rows($myQry1); ?>',
