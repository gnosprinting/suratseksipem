					...
					if ($ukuran_kk < (2*1024*1024)) {
						if (in_array($ekstensi_foto, $ekstensi_diperbolehkan) === true) {
							if ($ukuran_foto < (2*1024*1024)) {
								...
								//insert to database
								$insert = mysqli_query($koneksi, "INSERT INTO surat_ektp( nama, nik,kode_pos,keterangan,nomor_kk,alamat,s_pengantar,s_kk,s_foto,status_surat)
								VALUES('$nama','$nik','$kode_pos', '$keterangan', '$no_kk','$alamat','$s_pengantar','$s_kk','$s_foto','$status_surat')") or die (mysqli_error ($koneksi));
								if($insert){
  ...
  <div class="row">
    ...
    <div class="col">
			...
			<div class="form-group">
				...
				<input type="text" name="nomor_kk"  value="" class="form-control" placeholder="nomor kk" required>
			</div>
			<div class="form-group">
